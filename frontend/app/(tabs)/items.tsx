...
export default function ItemsScreen() {
  ...
  const fetchItems = async () => {
    try {
      ...
      setItems(
        response.success && Array.isArray(response.items) ? response.items : [],
      );
    } catch (err: any) {
    ...
    <View style={[styles.screen, isDark && styles.screenDark]}>
      <FlatList
        ...
        keyExtractor={(item, index) =>
          item._id || item.id || `${item.title}-${index}`
        }
        numColumns={2}
        ...
        ListHeaderComponent={
          <View>
            ...
            <Text style={[styles.summary, isDark && styles.mutedTextDark]}>
              {soldItems.length} {t("valuedItems")} · $
              {totalValue.toLocaleString()} {t("total")}
            </Text>
          ...
          <View style={styles.stateBlock}>
            {loading ? (
              <>
                ...
                <Text
                  style={[styles.stateText, isDark && styles.mutedTextDark]}
                >
                  {t("loadingItems")}
                </Text>
              </>
              ...
              <Text style={styles.stateText}>{t("noSoldItems")}</Text>
            )}
          </View>
        }
        renderItem={({ item }) => (
...
function SoldItemCard({
  ...
  return (
    ...
    >
      ...
      <Text
        numberOfLines={1}
        style={[styles.brand, isDark && styles.mutedTextDark]}
      >
      ...
      <View style={styles.cardFooter}>
        ...
        <Text style={[styles.confidence, isDark && styles.mutedTextDark]}>
          {confidence}%
        </Text>
      </View>
    </TouchableOpacity>
